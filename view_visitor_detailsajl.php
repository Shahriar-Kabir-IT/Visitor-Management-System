...
if(isset($_POST['update']))
{
  $eid=$_SESSION['editid'];
  $remark=$_POST['remark'];
  $query=mysqli_query($con,"update tblvisitorajl set remark='$remark' where  ID='$eid'");
  if ($query) {
    echo '<script>alert("Visitors Remark has been Updated.")</script>';
    echo "<script>window.location.href ='manage_visitorajl.php'</script>";
  }
  else{
...
<div class="card-body">
  <?php
  $eid=$_POST['edit_id5'];
  $sql="SELECT * from tblvisitorajl  where ID=:eid";
